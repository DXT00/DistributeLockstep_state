...
namespace Snappy.Sharp
{
    internal class Utilities
    {
        ...
        public unsafe static void UnalignedCopy64(byte[] source, int sourceIndex, byte[] dest, int destIndex)
        {
           ...
           dest[destIndex + 4] = source[sourceIndex + 4];
           dest[destIndex + 5] = source[sourceIndex + 5];
           dest[destIndex + 6] = source[sourceIndex + 6];
           dest[destIndex + 7] = source[sourceIndex + 7];
        ...
        public unsafe static uint GetFourBytes(byte[] source, int index)
        {
           ...
           /* fixed (byte* src = &source[index])
            ...
            uint uValue = 0;
            uValue |= ((uint)source[index]);
            uValue |= ((uint)source[index+1])<<8;
            uValue |= ((uint)source[index+2])<<16;
            uValue |= ((uint)source[index+3])<<24;
        ...
        public unsafe static ulong GetEightBytes(byte[] source, int index)
        {
           ...
           /* fixed (byte* src = &source[index])
            ...
            ulong uValue = 0;
            uValue |= ((ulong)source[index]);
            uValue |= ((ulong)source[index+1]) << 8;
            uValue |= ((ulong)source[index+2]) << 16;
            uValue |= ((ulong)source[index+3]) << 24;
            uValue |= ((ulong)source[index+4]) << 32;
            uValue |= ((ulong)source[index+5]) << 40;
            uValue |= ((ulong)source[index+6]) << 48;
            uValue |= ((ulong)source[index+7]) << 56;
